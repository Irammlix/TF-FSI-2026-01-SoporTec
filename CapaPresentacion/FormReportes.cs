...
namespace CapaPresentacion
{
    public partial class FormReportes : Form
    {
        ...
        // =========================================================================
        //  RF-19 — Tickets generados por sede y pabellón
        // =========================================================================
        private void ConstruirReporte4()
        {
            pnl_Reporte4.Controls.Clear();
            pnl_Reporte4.BackColor = ColorFondo;

            // ----- Contenido (se agrega primero para que el sidebar quede a la izquierda) -----
            Panel contenido = new Panel { Dock = DockStyle.Fill, BackColor = ColorFondo, Padding = new Padding(12) };
            pnl_Reporte4.Controls.Add(contenido);

            // ----- Sidebar navy -----
            Panel sidebar = new Panel { Dock = DockStyle.Left, Width = 110, BackColor = ColorNavy };
            sidebar.Controls.Add(CrearBotonSidebar("Salir", DockStyle.Bottom, (s, e) => this.Close()));
            sidebar.Controls.Add(CrearBotonSidebar("Limpiar\nFiltros", DockStyle.Top, (s, e) => LimpiarFiltros()));
            sidebar.Controls.Add(CrearBotonSidebar("Exportar\nPDF", DockStyle.Top, (s, e) => ExportarPdf()));
            sidebar.Controls.Add(CrearBotonSidebar("Refrescar", DockStyle.Top, (s, e) => CargarReporte()));
            pnl_Reporte4.Controls.Add(sidebar);

            // ----- Zona principal (chart+detalle | filtros) : se agrega primero (Fill) -----
            ...
            main.Controls.Add(CrearColumnaGrafico(), 0, 0);
            main.Controls.Add(CrearColumnaFiltros(), 1, 0);
            contenido.Controls.Add(main);

            // ----- Fila de KPIs (Top) -----
            contenido.Controls.Add(CrearFilaKpis());

            // ----- Título del reporte (Top, se agrega al final para quedar arriba) -----
            contenido.Controls.Add(CrearTitulo());

            // Cargar sedes y primer render
            CargarSedes();
            CargarReporte();
        }
